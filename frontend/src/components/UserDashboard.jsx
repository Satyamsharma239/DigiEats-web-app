...
function UserDashboard() {
  ...
  const handleFilterByCategory = (category) => {
    if (category === "All") {
      ...
      // FIXED: Case-insensitive match (Pizza == pizza)
  ...
  useEffect(() => {
    // Jab page load ho, toh saare items dikhao
    if (itemsInMyCity) {
      setUpdatedItemsList(itemsInMyCity)
    }
  }, [itemsInMyCity])
  ...
  return (
    <div className='w-screen min-h-screen flex flex-col gap-6 items-center bg-white overflow-x-hidden overflow-y-auto pb-10 pt-[110px]'>
      <Nav />
      {/* The new Hero Promo Banner */}
      <HeroBanner />
      ...
      <div className="w-full max-w-6xl flex flex-col gap-5 items-start p-[10px]">
        ...
        <h1 className='text-gray-900 text-2xl sm:text-[28px] font-black tracking-tight'>What's on your mind?</h1>
      ...
      <div className='w-full max-w-6xl flex flex-col gap-5 items-start p-[10px]'>
        <h1 className='text-gray-900 text-2xl sm:text-[28px] font-black tracking-tight'>Must-try restaurants in {currentCity}</h1>
        <div className='w-full relative'>
      ...
      <div className='w-full max-w-6xl flex flex-col gap-5 items-start p-[10px]'>
        <h1 className='text-gray-900 text-2xl sm:text-[28px] font-black tracking-tight'>
          Discover delicious dishes
        </h1>

        <div className='w-full h-auto flex flex-wrap gap-[20px] justify-center'>
          {/* Agar items list empty hai, toh user ko message dikhao */}
          {updatedItemsList?.length === 0 ? (
            <p className="text-gray-500">No items found in {currentCity}</p>
          ) : (
